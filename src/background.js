...
const update = async () => {
  ...
  const [{ id: queueFolderId }] = await browser.bookmarks.search({
    title: QUEUE_FOLDER_NAME,
  });
  const [{ id: archiveFolderId }] = await browser.bookmarks.search({
...
export const toggle = async tab => {
  ...
  if (queueFolderId && archiveFolderId) {
    ...
    if (foundBookmark && !foundArchive) {
      await browser.bookmarks.move(foundBookmark.id, {
        parentId: archiveFolderId,
      });
    } else if (foundBookmark && foundArchive)
      await browser.bookmarks.remove(foundBookmark.id);
    else if (!foundBookmark && foundArchive) {
      const cleanedTitle = foundArchive.title.substring(
        0,
        foundArchive.title.length - 15
      );
      await browser.bookmarks.update(foundArchive.id, { title: cleanedTitle });
      await browser.bookmarks.move(foundArchive.id, {
